...
global ERR_ODG, ERR_SYNTAX, POINT, PLATFORM, CHECK_UPDATE,DO_BUILD,BUILD_NAME,FONT1,HOVER_MASK,HOVER_MODE
ERR_ODG = "Not a odg file."
ERR_SYNTAX = "Syntax Error"
### préférences standard de certaines variables de l'UI 
PLATFORM = "MACOSX" # A CAUSE DE LA COMPATIBILITE DES CHEMIN DE FICHIERS : file path ... LINUX,RASPBERRY_PI,MACOSX,WINDOWS
...
### Function de la class
    ...
    def setParams(self,txt):
        global HOVER_MASK, PLATFORM, HOVER_MODE
        """"setObjects"""
        self.iniParams = dict()
        config = ini.ConfigParser()
        config.read(txt)
        sections = config.sections()
        if self.DEBUG == True: print sections
        ph =""
...
###     # definiton de paramètres importants
        try :
            ...
            if self.DEBUG == True: print "PLATFORM = ", PLATFORM
        except KeyError:
            pass
        try :
            HOVER_MASK = eval(self.iniParams['UI']['HOVER_MASK'])
            if self.DEBUG == True: print "HOVER_MASK = ", HOVER_MASK
        except KeyError:
            pass
        try :
            HOVER_MODE = self.iniParams['UI']['HOVER_MODE']
            if self.DEBUG == True: print "HOVER_MODE = ", HOVER_MODE
        except KeyError:
            pass
        try :
            self.APPPATH = self.iniParams['UI']['APPPATH']
            if self.DEBUG == True: print "APPPATH = ", self.APPPATH
        except KeyError:
            pass
        try :
            self.FULLSCREEN = eval(self.iniParams['UI']['FULLSCREEN'])
            if self.DEBUG == True: print "FULLSCREEN = ", self.FULLSCREEN
        except KeyError:
            pass
        try :
    ...
    def CreateHoverPng(self,nom,x,y,width,height,typeObject):
        ...
        crop_rectangle = (int(x)-1,int(y)-1,int(x)-1+int(width)+2,int(y)-1+int(height)+2)
        ...
        if HOVER_MODE == 'standard':
            ...
            cropped_im = ImageOps.colorize(cropped_im,black=HOVER_MASK,white="white")
            cropped_im.save("./ui/Pictures/"+nom+"_hover.png")
...
###     textctrl => les panneaux de textes
    ...
    def textctrl(self,uiObject,obj,nom):
        if self.DEBUG == True: print self.FONT
        if (nom[0]=="STD"):
            font = wx.Font(self.SIZE, wx.DEFAULT, wx.NORMAL, self.STYLE,face=self.FONT)
...
### la grosse routine de gestion des clicks  
    ...
    def initUI(self):
        ...
        if self.FULLSCREEN == True :
            self.window = wx.Frame(None, style= wx.FULL_REPAINT_ON_RESIZE)
            self.window.ShowFullScreen(True, style=wx.FULLSCREEN_ALL)
        else:
            self.window = wx.Frame(None, style= wx.FULL_REPAINT_ON_RESIZE | wx.SYSTEM_MENU | wx.CAPTION | wx.CLOSE_BOX)
        self.window.SetSize((self.uiPage['width'],self.uiPage['height']+20+10*self.FULLSCREEN))
